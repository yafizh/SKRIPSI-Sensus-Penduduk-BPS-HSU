                ...
                <div class="col">
                    <div class="title mb-30">
                        <?php if (count($periode_sensus)) : ?>
                            <h3>Data Penduduk (Periode Sensus <?= $periode_sensus[0]['tahun']; ?>)</h3>
                        <?php else : ?>
                            <h3>Data Penduduk</h3>
                        <?php endif; ?>
                    </div>
                        ...
                        <?php if (count($periode_sensus)) : ?>
                            <div class="table-responsive">
                                <table id="table" class="table">
                                    ...
                                    SELECT 
                                        *,
                                        (SELECT COUNT(*) FROM `kelurahan/desa` WHERE id_kecamatan=kecamatan.id) AS jumlah_kelurahan,
                                        (
                                            SELECT 
                                                COUNT(*)
                                            FROM 
                                            ...
                                            WHERE 
                                                k.id=kecamatan.id 
                                                AND 
                                                p.id_periode_sensus=" . $periode_sensus[0]['id'] . " 
                                        ) jumlah_rumah_tangga
                                    FROM 
                                        kecamatan 
                                    INNER JOIN 
                                        ...
                                        <?php if ($result->num_rows) : ?>
                                            <?php while ($row = $result->fetch_assoc()) : ?>
                                                <tr>
                                                    ...
                                                    <td class="fit">
                                                        <div class="action">
                                                            <a href="?page=data_sensus&sub_page=penduduk&action=detail_per_kecamatan&id_kecamatan=<?= $row['id']; ?>&id_periode_sensus=<?= $periode_sensus[0]['id']; ?>" class="text-secondary">
